    ...
    <div className="signup-body">
      <div className="signup-box">
        ...
        <form onSubmit={dataSubmission}>
          ...
          <input type="text" onChange={fnameErrHandler} />
          {fnameErr ? <span>Invaild name</span> : ""}

          <label>Last name</label>
          <input type="text" onChange={lnameErrHandler} />
          {lnameErr ? <span>Invalid name</span> : ""}

          <label>Email</label>
          <input type="email" onChange={emailErrHandler} />
          {emailErr ? <span></span> : ""}

          <label>Password</label>
          <input type="password" onChange={passErrHandler} />
          {passErr ? (
            <span>Password must contain minimum 8 charactors</span>
          ) : (
            ""
          )}

          <label>Confirm Password</label>
          <input type="password" onChange={cpassErrHandler} />
          {cpassErr ? <span>Password does not matched</span> : ""}

          <input type="button" value="Submit" />
        </form>
        <p>
          By clicking the Sign Up botton, you agree to our <br></br>
